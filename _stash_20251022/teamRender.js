...
async function handleLoginSuccess() {
  ...
  // --- If cached data exists, validate and maybe use it ---
  if (cached && cached.data) {
    const ageMs = Date.now() - (cached.timestamp || 0);
    const cacheAuthMode = cached.data?.debug?.authMode || 'unknown';
    const cacheEmail = cached.data?.debug?.emailParam || '';
    const isEditor = cached.data?.isTeamPageEditor ?? false;

    const cacheInvalidUser =
      cacheAuthMode === 'none' ||
      (cacheEmail && currentEmail && cacheEmail !== currentEmail) ||
      !isEditor;

    console.log(
      `handleLoginSuccess(): cache age=${Math.round(ageMs / 1000)}s, authMode=${cacheAuthMode}, cacheEmail=${cacheEmail}, isEditor=${isEditor}`
    );

    // --- Invalidate cache from anonymous, mismatched, or non-editor user ---
    if (cacheInvalidUser) {
      console.log(
        'Cache invalid — cached data from anonymous, mismatched, or non-editor user. Fetching fresh data.'
      );
      ...
      try {
        ...
        if (fresh?.success) {
          ...
          console.warn('Backend refresh failed or returned unsuccessful response:', fresh);
    ...
    // --- Render immediately from cache for fast page load ---
    console.log('handleLoginSuccess(): using cached data — rendering instantly.');
    renderTeamPage(cached.data);

    // Wait for DOM to settle before updating admin UI
    await new Promise(r => setTimeout(r, 250));
    refreshAdminUI(currentUser, cached.data);
    setLoading(false);

    // --- If cache is stale (>60 min), refresh silently in background ---
    if (ageMs > MAX_CACHE_AGE) {
      console.log('Cache older than 60 min — fetching fresh data in background.');
      (async () => {
        try {
          const fresh = await callBackend({ team, email: currentEmail });
          if (fresh?.success) {
  ...
  // --- No cache available — run full init ---
  console.log('handleLoginSuccess(): no cached data found — running full init.');
  await init(currentUser);
}

async function handleLogin() {
  console.log("handleLogin(): starting Google sign-in…");
  try {
    const result = await signInWithPopup(auth, provider);
    currentUser = result.user;
    console.log(`handleLogin(): signed in as ${currentUser.email}`);

    // Immediately trigger post-login refresh using cache or backend
    await handleLoginSuccess();

  } catch (err) {
    console.error("handleLogin(): sign-in failed:", err);
  }
}
...
function setupAuthUI() {
  onAuthStateChanged(auth, async (user) => {
    const wasInitRun = typeof initHasRun !== "undefined" && initHasRun;
    currentUser = user;
    const urlParams = new URLSearchParams(window.location.search);
    const team = urlParams.get('team') || '';
    ...
    if (user) {
      ...
      if (adminButtons) adminButtons.hidden = false;

      // Avoid re-running init() if it already completed once
      if (wasInitRun) {
        console.log("Auth listener: init() already ran, refreshing admin UI only.");
        if (window.lastRenderedTeamData) {
          await refreshAdminUI(user, window.lastRenderedTeamData);
        } else {
          console.log("No cached team data available, fetching fresh data...");
          await handleLoginSuccess(); // runs init() if needed
        }
        return;
      }
      ...
      const cacheAge = cached ? Math.floor((Date.now() - cached.timestamp) / 60000) : null;

      if (cached?.data && cacheAge < 60) {
        console.log(`Auth: user detected — using cached team data (${cacheAge} min old)`);

        await Promise.all([
          waitForElement('#adminButtons', 4000),
          waitForElement('#announcements', 4000)
        ]).catch(err => console.warn('Admin UI elements not ready:', err));

        await new Promise(r => setTimeout(r, 150));

        console.log("DOM ready — refreshing admin UI with cached data.");
        await refreshAdminUI(user, cached.data);
      } else {
        console.log("Auth: cache missing or too old — fetching new data via init().");
        await handleLoginSuccess(); // runs init()
      }

    } else {
      // --- User logged out ---
      console.log("Auth: logged out — refreshing public view only.");
      userInfo.innerText = '';
      
      // Keep elements in DOM, just hide them
      loginBtn.hidden = false;
      logoutBtn.hidden = true;

      const adminButtons = document.getElementById('adminButtons');
      if (adminButtons) adminButtons.hidden = true;

      // Optional: show a placeholder message or limited public view
      console.log('Admin UI elements hidden (not removed).');

      // Do NOT call backend again — let init() handle initial data load.
    }
  });
}
...
async function attachAuthButtonHandlers() {
  console.log("attachAuthButtonHandlers(): called");
  try {
    const loginBtn = await waitForElement('#loginBtn', 4000);
    const logoutBtn = await waitForElement('#logoutBtn', 4000);
    console.log("attachAuthButtonHandlers(): buttons found", { loginBtn, logoutBtn });

    if (loginBtn) {
      loginBtn.hidden = false;
      loginBtn.onclick = async () => {
        console.log("Login clicked — starting Google sign-in.");
        try {
          const result = await signInWithPopup(auth, provider);
          currentUser = result.user;
          console.log(`Login successful: ${currentUser.email}`);
          await refreshAdminUI(currentUser, window.lastRenderedTeamData);
        } catch (err) {
    ...
    if (logoutBtn) {
      logoutBtn.onclick = async () => {
        ...
        try {
          ...
          await refreshAdminUI(null, window.lastRenderedTeamData);
        } catch (err) {
    ...
    console.log("attachAuthButtonHandlers(): handlers attached successfully.");
  } catch (err) {
    console.error("attachAuthButtonHandlers(): failed:", err);
  }
}
...
async function init(user = currentUser) {
  ...
  // Attach auth button handlers first
  await attachAuthButtonHandlers();
...
async function refreshAdminUI(user = currentUser, data = window.lastRenderedTeamData) {
  ...
  let adminButtons, announcements;
  try {
    adminButtons = await waitForElement('#adminButtons', 4000);
    announcements = await waitForElement('#announcements', 4000);
  } catch (err) {
    console.warn("refreshAdminUI(): required elements not ready within timeout:", err);
    return;
  }

  if (!adminButtons || !announcements) {
    console.warn("refreshAdminUI(): missing required DOM nodes — aborting update.");
    return;
  }

  // --- Wait for auth buttons ---
  const loginBtn = await waitForElement('#loginBtn', 4000);
  const logoutBtn = await waitForElement('#logoutBtn', 4000);
  ...
  if (loginBtn) {
    ...
    console.warn("refreshAdminUI(): #loginBtn not found.");
  }
  ...
  if (logoutBtn) {
    logoutBtn.onclick = async () => {
      ...
      try {
        ...
        // Hide admin buttons, keep page content intact
        adminButtons.innerHTML = '';

        // Re-render announcements (removes edit/delete links)
        if (window.lastRenderedTeamData && typeof renderAnnouncements === 'function') {
          renderAnnouncements(window.lastRenderedTeamData);
        }
        ...
        console.log("Logout complete — page preserved.");
      } catch (err) {
        console.error("Logout failed:", err);
      }
    };
  } else {
    console.warn("refreshAdminUI(): #logoutBtn not found.");
  }

  // --- Determine admin/editor permissions ---
  const isAdmin = !!(user?.email && (data.isAdmin || data.isTeamPageEditor));
  console.log(`refreshAdminUI(): user is ${isAdmin ? '' : 'not '}an editor/admin`);

  // --- Toggle login/logout button visibility ---
  if (loginBtn && logoutBtn) {
    loginBtn.hidden = !!user; // hide login if logged in
    logoutBtn.hidden = !user; // hide logout if logged out
  }

  // --- Render admin buttons if admin ---
  if (isAdmin) {
    adminButtons.innerHTML = `
    ...
    adminButtons.innerHTML = ''; // non-admins see nothing
  }
  ...
  try {
    if (typeof renderAnnouncements === 'function') {
      ...
      console.warn("refreshAdminUI(): renderAnnouncements() not defined.");
    }
  } catch (err) {
    console.error("refreshAdminUI(): error re-rendering announcements:", err);
  }

  // --- Re-render the Update Page link (Google Form) ---
...
async function refreshAdminUIForLogout(data = window.lastRenderedTeamData) {
  if (!data) {
    console.warn("refreshAdminUIForLogout(): no data available yet.");
    return;
  }

  console.log("refreshAdminUIForLogout(): hiding admin controls, re-rendering announcements.");
  ...
  // Re-render announcements (without edit/delete links)
  const dataClone = { ...data, isTeamPageEditor: false, isAdmin: false };
  renderAnnouncements(dataClone);
}
...
export async function renderTeamPage(data) {
  ...
  const refreshBtn = await waitForElement('#refreshBtn', 3000);
  if (refreshBtn) {
    refreshBtn.style.display = data.isTeamPageEditor ? 'inline-block' : 'none';
  } else {
    console.warn('renderTeamPage(): #refreshBtn not found in DOM yet.');
  }
  ...
  const updateContainer = await waitForElement('#teamUpdateContainer', 3000);
  if (updateContainer) {
    ...
    console.warn('renderTeamPage(): #teamUpdateContainer not found in DOM yet.');
  }
...
async function handleLogout() {
  ...
  try {
    ...
    // DO NOT call init() or reload page — preserve the DOM
    console.log("Logout complete — UI cleaned but page preserved.");
  } catch (err) {
    console.error("Logout failed:", err);
  }
}